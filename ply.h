...
namespace PLY {
	...
	void ReadPLYFiles(std::string path);
	/////////////////////////////////////////
...
#define BIG_STRING 4096
	...
	******************************************************************************/
	...
	void ReadPLYFiles(std::string filename,
		...
		std::vector<std::array<unsigned, 6>> & faces)
	{
		...
		ply = ply_open_for_reading(cstr, &nelems, &elist, &file_type, &version);
		...
		for (i = 0; i < nelems; i++) {
			...
			if (equal_strings("vertex", elem_name)) {
				...
				for (j = 0; j < num_elems; j++) {
					...
					positions.emplace_back(vlist[j]->x, vlist[j]->y, vlist[j]->z);

					//todo change this 

					normals.emplace_back(vlist[j]->x, vlist[j]->y, vlist[j]->z);
				}
			}
			...
			if (equal_strings("face", elem_name)) {
				...
				for (j = 0; j < num_elems; j++) {
					...
					for (k = 0; k < flist[j]->nverts - 2; k++)
						faces.emplace_back(std::array<unsigned, 6>{ unsigned(flist[j]->verts[0]), (0), unsigned(flist[j]->verts[k + 1]), unsigned(0), unsigned(flist[j]->verts[k + 2]), unsigned(0) });
