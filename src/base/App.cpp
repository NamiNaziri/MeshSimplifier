...
namespace {
		...
		const vector<array<unsigned, 6>> & faces)
	{
		...
		//TODO Change thiiiiiiiiiiiiiiiiiiiiis
		...
		for (auto& f : faces) {
			...
			for (int i = 0; i < 6; i++)
			{
				if (i % 2 == 0) // position
				{
					tri[i / 2].position = positions[f[i]];
				}
				else
				{
					tri[i / 2].normal = normals[f[i]];
				}
			}

			std::move(tri.begin(), tri.end(), std::back_inserter(vertices));
		}
	...
	vector<Vertex> loadUserGeneratedModel() {
		...
		for (auto i = 0u; i < faces; ++i) {
			...
			v0.normal = v1.normal = v2.normal = cross((v1.position - v0.position), (v2.position - v0.position)).normalized();
			//
			// Some hints:
...
App::App(void)
	...
	camera_trackball_changed_(false),
	camera_trackball_(false),
	rotation_animation_(false),
	rotation_animation_changed(false),
	camera_rotation_angle_(0.0f),
	translation_(0.0f),
	movement_velocity_(0.1f),
	rotation_angle_around_y_(0.f),
	scale_along_x_(1.f),
	left_mouse_button_pressed_(false),
	win_FOV_(90),
	simplify_(false),
	simplify_changed(false),
	simplify1000_changed(false),
	simplify_1000(false)

{
	width_ = 800;
	height_ = (800);
	Hemisphere h(width_, height_);
	virtual_trackball_camera_ = VirtualTrackballCamera(h, FW::Vec2f(0, 0));
	...
	common_ctrl_.addToggle(&camera_trackball_, FW_KEY_O, "Trackball Camera (O)", &camera_trackball_changed_);
	common_ctrl_.addSeparator();
	common_ctrl_.addToggle(&rotation_animation_, FW_KEY_I, "Rotate Animation (I)", &rotation_animation_changed);
	common_ctrl_.addToggle(&simplify_, FW_KEY_S, "Simplify (S)", &simplify_changed);
	common_ctrl_.addToggle(&simplify_1000, FW_KEY_L, "Simplify to  1000 faces (l)", &simplify1000_changed);
	...
	window_.setSize(Vec2i(width_, height_));
}
...
bool App::handleEvent(const Window::Event& ev) {
	if (model_changed_) {
		...
		case MODEL_FROM_FILE:
		{
			...
			if (filename.getLength()) {
				if (std::string(filename.getPtr()).find("obj") != std::string::npos)
				{
					streamGeometry(loadObjFileModel(filename.getPtr()));
				}
				else if (std::string(filename.getPtr()).find("ply") != std::string::npos)
				{
	...
	if (camera_trackball_changed_)
	{
		cout << "FW_KEY_MOUSE_LEFT" << "(" << ev.mousePos.x << "," << ev.mousePos.y << endl << endl;
		virtual_trackball_camera_.ResetCamera(Vec2i(0, 0));
		camera_trackball_changed_ = false;

	}

	if (rotation_animation_changed)
	{
	...
	if (simplify_changed)
	{
		...
		if (simplify_)
		{
			...
			this->Simplify(this->faces.size() * 0.9);
	...
	if (ev.type == Window::EventType_KeyDown) {
		...
		if (ev.key == FW_KEY_UP)
		{
			translation_ += movement_velocity_ * FW::Vec3f(0.f, 1.f, 0.f);
		}
		...
		{
			translation_ += movement_velocity_ * FW::Vec3f(1.f, 0.f, 0.f);
		}
		...
		{
			scale_along_x_ += movement_velocity_;
		}
		...
		{
			rotation_angle_around_y_ += movement_velocity_ * FW_PI;
		}
		...
		else if (ev.key == FW_KEY_MOUSE_LEFT)
		{
			...
			virtual_trackball_camera_.ResetCamera(ev.mousePos);
		}
	}
	...
	{
		width_ = ev.window->getSize().x;
		height_ = ev.window->getSize().y;
	}
	...
	if (ev.type == Window::EventType_Mouse) {
		...
		if (ev.mouseDragging && left_mouse_button_pressed_ && camera_trackball_)
		{
			virtual_trackball_camera_.UpdateCamera(ev.mousePos);
...
void App::initRendering() {
	...
	uniform mat3 uModelToWorldInverseTranspose;
	...
	void main()
	{
		...
		vec3 transformedNormal = uModelToWorldInverseTranspose * aNormal;
		float clampedCosine = clamp(dot(transformedNormal, directionToLight), 0.0, 1.0);
	...
	gl_.model_to_world_inverse_transpose_uniform = glGetUniformLocation(gl_.shader_program, "uModelToWorldInverseTranspose");
...
void App::render() {
	...
	Mat4f C = virtual_trackball_camera_.GetCameraTransform();
	Mat4f rot = CustomMath::ToMat4(Mat3f::rotation(Vec3f(0, 1, 0), -camera_rotation_angle_));
	C = rot * C;
	C.setCol(3, Vec4f(0, 0, camera_distance, 1));



	const float ar = width_ / height_;
	const float tanHalfFOV = tanf(CustomMath::ToRadian(win_FOV_ / 2.0));
	// Simple perspective.
	static const float fnear = 0.1f, ffar = 16.0f;
	Mat4f P;
	P.setCol(0, Vec4f(1.0f / (tanHalfFOV * ar), 0, 0, 0));
	P.setCol(1, Vec4f(0, 1.0f / tanHalfFOV, 0, 0));
	...
	Mat4f modelToWorld;
	Mat4f scale(Mat4f::scale(Vec3f(scale_along_x_, scale_along_x_, scale_along_x_)));
	Mat4f rotation(CustomMath::Rotation(Vec3f(0, 1, 0), rotation_angle_around_y_));
	Mat4f translation(Mat4f::translate(translation_));
	modelToWorld = translation * scale * rotation * modelToWorld;
	...
	Mat3f modelToWorldInverseTranspose = CustomMath::FromMat4(modelToWorld.inverted().transposed());


	glUniformMatrix3fv(gl_.model_to_world_inverse_transpose_uniform, 1, GL_FALSE, modelToWorldInverseTranspose.getPtr());
...
vector<Vertex> App::loadObjFileModel(string filename) {
	...
	while (getline(input, line)) {
		...
		if (pos != std::string::npos) {
			...
			{
				line.replace(pos, toReplace.length(), " ");
	...
	this->faces = faces;
	S = QEM_SurfaceSimplification();
	S.SetVariables(faces, positions, normals);
...
void FW::App::SimplificationAlgorithm()
{

}

void FW::App::Simplify(int fNum)
{
	S.Simplify(fNum);
	...
	for (auto& f : this->faces) {
		...
		for (int i = 0; i < 6; i++)
		{
			if (i % 2 == 0) // position
			{
				tri[i / 2].position = this->positions[f[i]];
			}
			else
			{
				tri[i / 2].normal = this->normals[f[i]];
			}
		}

		std::move(tri.begin(), tri.end(), std::back_inserter(vertices));
	}
	streamGeometry(vertices);
}

std::vector<Vertex> FW::App::loadPlyFileModel(std::string filename)
{
	window_.showModalMessage(sprintf("Loading mesh from '%s'...", filename.c_str()));

	vector<Vec3f> positions, normals;
	vector<array<unsigned, 6>> faces;
	PLY::ReadPLYFiles(filename,positions,normals,faces);
	this->positions = positions;
	this->normals = normals;
	this->faces = faces;
	S = QEM_SurfaceSimplification();
